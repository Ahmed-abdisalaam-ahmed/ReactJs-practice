import {useEffect, useState} from 'react'
import {useLocation, useNavigate } from 'react-router-dom';
import data from "./data/100.json";

const movies =[
    {
        "id": "tt31193180",
        "url": "https://www.imdb.com/title/tt31193180/",
        "primaryTitle": "Sinners",
        "originalTitle": "Sinners",
        "type": "movie",
        "description": "Trying to leave their troubled lives behind, twin brothers return to their hometown to start again, only to discover that an even greater evil is waiting to welcome them back.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BNjIwZWY4ZDEtMmIxZS00NDA4LTg4ZGMtMzUwZTYyNzgxMzk5XkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=bKGxHflevuk",
        "contentRating": "R",
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-04-18",
        "interests": [
            "Period Drama",
            "Supernatural Horror",
            "Vampire Horror",
            "Action",
            "Drama",
            "Horror",
            "Thriller"
        ],
        "countriesOfOrigin": [
            "US",
            "AU",
            "CA"
        ],
        "externalLinks": [
            "https://amzn.to/4cNibD5",
            "https://www.sinnersmovie.com/"
        ],
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "New Orleans, Louisiana, USA"
        ],
        "productionCompanies": [
            {
                "id": "co0002663",
                "name": "Warner Bros."
            },
            {
                "id": "co1090055",
                "name": "Domain Entertainment (II)"
            },
            {
                "id": "co0843639",
                "name": "Proximity Media"
            }
        ],
        "budget": 90000000,
        "grossWorldwide": 163235091,
        "genres": [
            "Action",
            "Drama",
            "Horror"
        ],
        "isAdult": false,
        "runtimeMinutes": 137,
        "averageRating": 8.2,
        "numVotes": 55596,
        "metascore": 84
    },
    {
        "id": "tt14123284",
        "url": "https://www.imdb.com/title/tt14123284/",
        "primaryTitle": "Havoc",
        "originalTitle": "Havoc",
        "type": "movie",
        "description": "After a drug deal gone wrong, a bruised detective must fight his way through the criminal underworld to rescue a politician's estranged son, unraveling a deep web of corruption and conspiracy that ensnares his entire city.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BZWJjMjdhNGEtZjEyMi00OTg2LTllMzQtN2JlZWNjYzI4YzFhXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=6txjTWLoSc8",
        "contentRating": "TV-MA",
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-04-25",
        "interests": [
            "Conspiracy Thriller",
            "Drug Crime",
            "Gun Fu",
            "Martial Arts",
            "One-Person Army Action",
            "Suspense Mystery",
            "Action",
            "Crime",
            "Drama",
            "Mystery"
        ],
        "countriesOfOrigin": [
            "GB",
            "US"
        ],
        "externalLinks": [
            "https://www.netflix.com/title/81330790"
        ],
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "South Wales, UK"
        ],
        "productionCompanies": [
            {
                "id": "co0614405",
                "name": "One More One Productions"
            },
            {
                "id": "co0354803",
                "name": "Severn Screen"
            },
            {
                "id": "co0244345",
                "name": "XYZ Films"
            }
        ],
        "budget": null,
        "grossWorldwide": null,
        "genres": [
            "Action",
            "Crime",
            "Drama"
        ],
        "isAdult": false,
        "runtimeMinutes": 107,
        "averageRating": 5.9,
        "numVotes": 20843,
        "metascore": 58
    },
    {
        "id": "tt20215234",
        "url": "https://www.imdb.com/title/tt20215234/",
        "primaryTitle": "Conclave",
        "originalTitle": "Conclave",
        "type": "movie",
        "description": "When Cardinal Lawrence is tasked with leading one of the world's most secretive and ancient events, selecting a new Pope, he finds himself at the center of a web of conspiracies and intrigue that could shake the very foundation of the Catholic Church.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BYjgxMDI5NmMtNTU3OS00ZDQxLTgxZmEtNzY1ZTBmMDY4NDRkXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=JX9jasdi3ic",
        "contentRating": "PG",
        "startYear": 2024,
        "endYear": null,
        "releaseDate": "2024-10-25",
        "interests": [
            "Conspiracy Thriller",
            "Drama",
            "Thriller"
        ],
        "countriesOfOrigin": [
            "GB",
            "US"
        ],
        "externalLinks": [
            "https://cclv-movie.jp/"
        ],
        "spokenLanguages": [
            "en",
            "it",
            "es",
            "la"
        ],
        "filmingLocations": [
            "Cinecittà Studios, Cinecittà, Rome, Lazio, Italy"
        ],
        "productionCompanies": [
            {
                "id": "co0215519",
                "name": "Indian Paintbrush"
            },
            {
                "id": "co0635417",
                "name": "Access Entertainment"
            },
            {
                "id": "co0251858",
                "name": "FilmNation Entertainment"
            }
        ],
        "budget": 20000000,
        "grossWorldwide": 116366167,
        "genres": [
            "Drama",
            "Thriller"
        ],
        "isAdult": false,
        "runtimeMinutes": 120,
        "averageRating": 7.4,
        "numVotes": 162780,
        "metascore": 79
    },
    {
        "id": "tt3566834",
        "url": "https://www.imdb.com/title/tt3566834/",
        "primaryTitle": "A Minecraft Movie",
        "originalTitle": "A Minecraft Movie",
        "type": "movie",
        "description": "Four misfits are suddenly pulled through a mysterious portal into a bizarre cubic wonderland that thrives on imagination. To get back home they'll have to master this world while embarking on a quest with an unexpected expert crafter.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BYzFjMzNjOTktNDBlNy00YWZhLWExYTctZDcxNDA4OWVhOTJjXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=wJO_vIDZn-I",
        "contentRating": "PG",
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-04-04",
        "interests": [
            "Action Epic",
            "Adventure Epic",
            "Fantasy Epic",
            "High-Concept Comedy",
            "Quest",
            "Slapstick",
            "Survival",
            "Action",
            "Adventure",
            "Comedy"
        ],
        "countriesOfOrigin": [
            "SE",
            "US",
            "NZ",
            "CA"
        ],
        "externalLinks": [
            "https://www.minecraft-movie.com/home/",
            "https://www.youtube.com/watch?v=EaOnM8SewHc&ab"
        ],
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "Auckland, New Zealand"
        ],
        "productionCompanies": [
            {
                "id": "co0002663",
                "name": "Warner Bros."
            },
            {
                "id": "co0159111",
                "name": "Legendary Entertainment"
            },
            {
                "id": "co0084207",
                "name": "Vertigo Entertainment"
            }
        ],
        "budget": 150000000,
        "grossWorldwide": 815953914,
        "genres": [
            "Action",
            "Adventure",
            "Comedy"
        ],
        "isAdult": false,
        "runtimeMinutes": 101,
        "averageRating": 5.9,
        "numVotes": 74763,
        "metascore": 45
    },
    {
        "id": "tt7068946",
        "url": "https://www.imdb.com/title/tt7068946/",
        "primaryTitle": "The Accountant 2",
        "originalTitle": "The Accountant 2",
        "type": "movie",
        "description": "Christian Wolff applies his brilliant mind and illegal methods to reconstruct the unsolved puzzle of a Treasury chief's murder.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BYjFiY2I0NTEtNjdjNy00MTljLThkOWYtMzNiYjYyMDcxYjhkXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=3wRCOqyDI6E",
        "contentRating": "R",
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-04-25",
        "interests": [
            "Whodunnit",
            "Action",
            "Crime",
            "Drama",
            "Mystery",
            "Thriller"
        ],
        "countriesOfOrigin": [
            "US"
        ],
        "externalLinks": null,
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "Los Angeles, California, USA"
        ],
        "productionCompanies": [
            {
                "id": "co1025982",
                "name": "Amazon MGM Studios"
            },
            {
                "id": "co0961831",
                "name": "Artists Equity"
            },
            {
                "id": "co0655304",
                "name": "51 Entertainment"
            }
        ],
        "budget": 80000000,
        "grossWorldwide": 37733959,
        "genres": [
            "Action",
            "Crime",
            "Drama"
        ],
        "isAdult": false,
        "runtimeMinutes": 132,
        "averageRating": 7.2,
        "numVotes": 9115,
        "metascore": 58
    },
    {
        "id": "tt31434639",
        "url": "https://www.imdb.com/title/tt31434639/",
        "primaryTitle": "Warfare",
        "originalTitle": "Warfare",
        "type": "movie",
        "description": "A platoon of Navy SEALs embark on a dangerous mission in Ramadi, Iraq, with the chaos and brotherhood of war retold through their memories of the event.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BYzEyYjE1NmEtOTFmNy00ZmQxLThlYzctOGRjNmQ0N2VjMmNmXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=JER0Fkyy3tw",
        "contentRating": "R",
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-04-11",
        "interests": [
            "Docudrama",
            "War Epic",
            "Action",
            "Drama",
            "War"
        ],
        "countriesOfOrigin": [
            "US",
            "GB"
        ],
        "externalLinks": [
            "https://a24films.com/films/warfare"
        ],
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "Iraq"
        ],
        "productionCompanies": [
            {
                "id": "co0390816",
                "name": "A24"
            },
            {
                "id": "co0103974",
                "name": "DNA Films"
            }
        ],
        "budget": 20000000,
        "grossWorldwide": 24595682,
        "genres": [
            "Action",
            "Drama",
            "War"
        ],
        "isAdult": false,
        "runtimeMinutes": 95,
        "averageRating": 7.9,
        "numVotes": 14994,
        "metascore": 78
    },
    {
        "id": "tt31227572",
        "url": "https://www.imdb.com/title/tt31227572/",
        "primaryTitle": "Predator: Badlands",
        "originalTitle": "Predator: Badlands",
        "type": "movie",
        "description": "A young Predator outcast from his clan finds an unlikely ally on his journey in search of the ultimate adversary.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BZTgxMzllM2YtZDMyNS00YmJlLWE4MGEtNDNjZTA5YjAwNzgwXkEyXkFqcGc@._V1_.jpg",
        "trailer": null,
        "contentRating": null,
        "startYear": 2025,
        "endYear": null,
        "releaseDate": "2025-11-07",
        "interests": [
            "Alien Invasion",
            "Sci-Fi"
        ],
        "countriesOfOrigin": null,
        "externalLinks": null,
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": null,
        "productionCompanies": [
            {
                "id": "co0781821",
                "name": "20th Century Studios"
            }
        ],
        "budget": null,
        "grossWorldwide": null,
        "genres": [
            "Horror",
            "Sci-Fi"
        ],
        "isAdult": false,
        "runtimeMinutes": null,
        "averageRating": null,
        "numVotes": null,
        "metascore": null
    },
    {
        "id": "tt18412256",
        "url": "https://www.imdb.com/title/tt18412256/",
        "primaryTitle": "Alien: Romulus",
        "originalTitle": "Alien: Romulus",
        "type": "movie",
        "description": "While scavenging the deep ends of a derelict space station, a group of young space colonists come face to face with the most terrifying life form in the universe.",
        "primaryImage": "https://m.media-amazon.com/images/M/MV5BMDU0NjcwOGQtNjNjOS00NzQ3LWIwM2YtYWVmODZjMzQzN2ExXkEyXkFqcGc@._V1_.jpg",
        "trailer": "https://www.youtube.com/watch?v=OzY2r2JXsDM",
        "contentRating": "R",
        "startYear": 2024,
        "endYear": null,
        "releaseDate": "2024-08-16",
        "interests": [
            "Monster Horror",
            "Space Sci-Fi",
            "Horror",
            "Sci-Fi",
            "Thriller"
        ],
        "countriesOfOrigin": [
            "GB",
            "US",
            "HU",
            "AU",
            "NZ",
            "CA"
        ],
        "externalLinks": [
            "https://amzn.to/3BLv4PB",
            "https://www.20thcenturystudios.com/movies/alien-romulus"
        ],
        "spokenLanguages": [
            "en"
        ],
        "filmingLocations": [
            "Origo Studios, Budapest, Hungary"
        ],
        "productionCompanies": [
            {
                "id": "co0781821",
                "name": "20th Century Studios"
            },
            {
                "id": "co0074212",
                "name": "Scott Free Productions"
            },
            {
                "id": "co0132161",
                "name": "Brandywine Productions"
            }
        ],
        "budget": 80000000,
        "grossWorldwide": 350865342,
        "genres": [
            "Horror",
            "Sci-Fi",
            "Thriller"
        ],
        "isAdult": false,
        "runtimeMinutes": 119,
        "averageRating": 7.1,
        "numVotes": 245461,
        "metascore": 64
    }
]



  const posts = [
  { id: 1, title: "React Basics", content: "Learn the basics of React.js" },
  { id: 2, title: "React Router", content: "Using React Router for navigation" },
  { id: 3, title: "Hooks in React", content: "Understanding React Hooks" },
  { id: 4, title: "JavaScript Fundamentals", content: "Core concepts of JS" },
];

  const useQuery = () => {
    return new URLSearchParams(useLocation().search); // to get the query params
  }


const Courses = () => {
  
  const query = useQuery(); 
  
  const searchTerm = query.get('search') || ''; // get 'search' param value

  const [search , setSearch] = useState(searchTerm);

  const Navigate = useNavigate();

  useEffect(()=>{
    if(search){
      Navigate(`?search=${search}`);
    }else{
      Navigate('');
    }
  },[search , Navigate])

  const filteredPosts = data.filter(post=> post.primaryTitle.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="max-w-xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-xl font-semibold mb-4">Search Posts</h2>
      <input
        type="text"
        value={search}
        onChange={(e) => setSearch(e.target.value)} // update search state 
        placeholder="Search posts..."
        className="w-full p-2 border border-gray-300 rounded-md"
      />

      <div className="mt-4">
        {filteredPosts.length > 0 ? (
          filteredPosts.map((post) => (
            <div key={post.id} className="p-4 bg-white rounded-2xl shadow-md grid grid-cols-4 mb-4">
              {/* <h3 className="text-lg font-bold">{post.title}</h3>
              <p className="text-gray-700">{post.content}</p> */}
        <p className='text-lg font-semibold text-gray-800'>{post.primaryTitle}</p>
        <img src={post.primaryImage} alt="image" 
        className='w-56 h-40 rounded-lg shadow-sm'
        />
            </div>
          ))
        ) : (
          <p className="text-red-500 font-semibold mt-4">No posts found</p>
        )}
      </div>

    </div>
  )
}

export default Courses